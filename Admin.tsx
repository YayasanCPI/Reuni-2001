  ...
  return (
    <div className="min-h-screen bg-paper-500 font-sans text-navy-900 pb-20">
      ...
      <div className="max-w-4xl mx-auto px-4 mt-8 space-y-12">
        ...
        {/* Schedule */}
        <section className="bg-paper-200 p-6 shadow-md border-2 border-navy-900 relative">
          <h2 className="text-2xl font-marker mb-6">Jadwal Acara</h2>
          <div className="space-y-6">
            {formData.schedule.map((item, idx) => (
              <div key={idx} className="bg-paper-100 p-4 border border-navy-900 relative">
                <button onClick={() => {
                ...
                <div className="grid md:grid-cols-2 gap-4 mb-4 pr-8">
                  <div>
                    ...
                    <input type="text" value={item.time} onChange={e => {
                      const newSch = [...formData.schedule];
                      newSch[idx].time = e.target.value;
                      setFormData({...formData, schedule: newSch});
                    }} className="w-full px-3 py-2 bg-white border border-navy-900" placeholder="08:00 - 10:00" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold font-serif mb-1">Judul Acara</label>
                    <input type="text" value={item.title} onChange={e => {
                      const newSch = [...formData.schedule];
                      newSch[idx].title = e.target.value;
                      setFormData({...formData, schedule: newSch});
                    }} className="w-full px-3 py-2 bg-white border border-navy-900" />
                  </div>
                  <div className="md:col-span-2">
                    <label className="block text-xs font-bold font-serif mb-1">Lokasi</label>
                    <input type="text" value={item.location} onChange={e => {
                      const newSch = [...formData.schedule];
                      newSch[idx].location = e.target.value;
                      setFormData({...formData, schedule: newSch});
                    }} className="w-full px-3 py-2 bg-white border border-navy-900" />
                  </div>
                  <div className="md:col-span-2">
                    <label className="block text-xs font-bold font-serif mb-1">Deskripsi</label>
                    <textarea value={item.description} onChange={e => {
                      const newSch = [...formData.schedule];
                      newSch[idx].description = e.target.value;
                      setFormData({...formData, schedule: newSch});
                    }} rows={2} className="w-full px-3 py-2 bg-white border border-navy-900" />
                  </div>
                </div>
              </div>
            ))}
            <button onClick={() => {
              setFormData({...formData, schedule: [...formData.schedule, { time: '', title: '', description: '', location: '' }]});
            }} className="flex items-center gap-2 text-navy-700 font-bold font-serif hover:text-navy-900">
